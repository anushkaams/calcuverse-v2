...
export default function CalculatorsPage({ params }: { params: Promise<{ category: string }> }) {
  const { category: categoryParam } = use(params);

  if (!VALID_CATEGORIES.includes(categoryParam)) notFound();

  const router = useRouter();
  const initialCategory = categoryParam === "all" ? "all" : (categoryParam as CalcCategory);
  const firstInCat = CALCULATORS.find(c => initialCategory === "all" || c.category === initialCategory);

  const [activeId, setActiveId] = useState(firstInCat?.id ?? CALCULATORS[0].id);
  ...
  const handleCalcSelect = (id: string) => {
    setActiveId(id);
    setSidebarOpen(false);
    router.push(`/calculator/${id}`, { scroll: false });
  };
