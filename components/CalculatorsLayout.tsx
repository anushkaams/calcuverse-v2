...
export default function CalculatorsLayout({ activeId: initialActiveId, initialCategory = "all" }: CalculatorsLayoutProps) {
  const router = useRouter();

  const firstInCat = CALCULATORS.find(c => initialCategory === "all" || c.category === initialCategory);
  const defaultId = initialActiveId ?? firstInCat?.id ?? CALCULATORS[0].id;

  const [activeId, setActiveId] = useState(defaultId);
  const [search, setSearch] = useState("");
  const [category, setCategory] = useState<CalcCategory | "all">(initialCategory);
  ...
  const activeCalc = CALCULATORS.find(c => c.id === activeId) ?? CALCULATORS[0];
  ...
  const handleCategoryChange = (c: CalcCategory | "all") => {
    ...
    if (first) {
      ...
      // Navigate to the dedicated page for this calculator (for metadata + URL)
