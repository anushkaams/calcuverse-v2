...
function CategoryFilter({
  ...
  const all = [
    ...
    ...Object.entries(CATEGORY_LABELS).map(([id, label]) => ({
      id,
      label,
      color: CATEGORY_COLORS[id as CalcCategory],
    })),
  ];
  return (
    <nav className="space-y-0.5">
      {all.map((cat) => (
        <button
          key={cat.id}
          onClick={() => onChange(cat.id as CalcCategory | "all")}
          ...
          <span className="flex items-center gap-2.5">
            <span
              ...
              style={{
                ...
                opacity: active === cat.id ? 1 : 0.5,
              }}
            />
            {cat.label}
          </span>
          <span
            className={`text-xs ${active === cat.id ? "text-[var(--text-secondary)]" : "text-[var(--text-muted)]"}`}
          >
            {counts[cat.id] ?? 0}
          </span>
        </button>
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <span
          className="text-xl w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 text-base"
        ...
        <div className="min-w-0">
          ...
          <div className="text-xs text-[var(--text-muted)] mt-0.5 leading-snug line-clamp-1">
            {calc.description}
          </div>
        </div>
      </div>
...
interface CalculatorsLayoutProps {
  /** The calculator id to show on first render (e.g. "age", "fuel") */
  activeId?: string;
  /** The category to pre-select (e.g. "date-time", "all") */
  initialCategory?: CalcCategory | "all";
}
...
export default function CalculatorsLayout({
  ...
  const firstInCat = CALCULATORS.find(
    (c) => initialCategory === "all" || c.category === initialCategory,
  );
  const defaultId = initialActiveId ?? firstInCat?.id ?? CALCULATORS[0].id;

  const [activeId, setActiveId] = useState(defaultId);
  const [search, setSearch] = useState("");
  const [category, setCategory] = useState<CalcCategory | "all">(
    initialCategory,
  );
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const calcListRef = useRef<HTMLDivElement>(null);
  ...
  const handleCategoryChange = (c: CalcCategory | "all") => {
    setCategory(c);
    setSearch("");
    const first = CALCULATORS.find(
      (calc) => c === "all" || calc.category === c,
    );
    if (first) {
      setActiveId(first.id);
      router.push(`/calculators/${first.slug}`, { scroll: false });
    }
    // Scroll calc list into view on mobile after a short delay (let render settle)
    setTimeout(() => {
      calcListRef.current?.scrollIntoView({
        behavior: "smooth",
        block: "start",
      });
    }, 50);
  ...
  const handleCalcSelect = (calc: CalculatorMeta) => {
    setActiveId(calc.id);
    setSidebarOpen(false);
    // Navigate to the dedicated SEO page for this calculator
    router.push(`/calculators/${calc.slug}`, { scroll: false });
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          <button
            className="ml-auto lg:hidden p-1 rounded text-[var(--text-muted)] hover:text-[var(--text-primary)]"
            onClick={(e) => {
              e.preventDefault();
              setSidebarOpen(false);
            }}
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        >
          <button
            className="lg:hidden p-1.5 rounded-lg border text-sm"
            style={{
              borderColor: "var(--border)",
